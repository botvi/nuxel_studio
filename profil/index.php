...
<body>
    ...
    <div id="desktop-wrapper">
        <div id="mobile-frame">
            ...
            <div id="game-container">
                <img id="profil-gif-page" src="../assets/image/ui/profil.gif" alt="profil">
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        class ProfileScene extends Phaser.Scene {
            ...
            preload() {
                ...
                this.load.image('kayuah', '../assets/image/ui/kayuah.png');
            ...
            create() {
                ...
                const drawProgressBar = (yPos, percent) => {
                    const barW = cW - 60;
                    const barH = 8;
                    cardBg.fillStyle(0x15803d, 0.15);
                    cardBg.fillRoundedRect(-barW / 2, yPos, barW, barH, 4);
                    cardBg.lineStyle(1.5, 0x15803d, 0.3);
                    cardBg.strokeRoundedRect(-barW / 2, yPos, barW, barH, 4);

                    cardBg.fillStyle(0x22c55e, 1);
                    cardBg.fillRoundedRect(-barW / 2, yPos, barW * percent, barH, 4);
                };

                // Render progress bars
                drawProgressBar(45, 0.92);  // Kecepatan
                drawProgressBar(95, 0.88);  // Kekuatan
                drawProgressBar(145, 0.95); // Ketukan
                ...
                const badgeTxt = this.add.text(0, -30, '⚡ CORE ROWERS ⚡', {
                    fontFamily: '"Press Start 2P", monospace',
                    fontSize: '8px',
                    color: '#ffffff'
                }).setOrigin(0.5);
                ...
                // Label Stats & Nilai
                const labelStyle = { fontFamily: '"Pixelify Sans", monospace', fontSize: '13px', fontStyle: 'bold', color: '#15803d' };
                const valueStyle = { fontFamily: '"Press Start 2P", monospace', fontSize: '9px', color: '#d97706' };

                // 1. Kecepatan
                const speedLbl = this.add.text(-cW / 2 + 30, 30, 'KECEPATAN', labelStyle).setOrigin(0, 0.5);
                const speedVal = this.add.text(cW / 2 - 30, 30, '92%', valueStyle).setOrigin(1, 0.5);
                cardContainer.add([speedLbl, speedVal]);

                // 2. Kekuatan
                const staminaLbl = this.add.text(-cW / 2 + 30, 80, 'KEKUATAN', labelStyle).setOrigin(0, 0.5);
                const staminaVal = this.add.text(cW / 2 - 30, 80, '88%', valueStyle).setOrigin(1, 0.5);
                cardContainer.add([staminaLbl, staminaVal]);

                // 3. Ketukan
                const rhythmLbl = this.add.text(-cW / 2 + 30, 130, 'KETUKAN', labelStyle).setOrigin(0, 0.5);
                const rhythmVal = this.add.text(cW / 2 - 30, 130, '95%', valueStyle).setOrigin(1, 0.5);
                cardContainer.add([rhythmLbl, rhythmVal]);

                // Extra stats at bottom
                // Extra stats at bottom
                const bottomStats = this.add.text(0, 185, '🏆 MENANG: 42  |  ❌ KALAH: 12', {
                    fontFamily: '"Pixelify Sans", monospace',
                    fontSize: '12px',
                    fontStyle: 'bold',
                    color: '#16a34a'
                ...
                // =====================================================
                //  PREVIEW JALUAR (DI BAWAH KARTU PROFIL)
                // =====================================================
                const boxY = H - 80;
                const previewBoxContainer = this.add.container(cx, boxY);

                const boxBg = this.add.graphics();
                boxBg.fillStyle(0x15803d, 0.25);
                boxBg.fillRoundedRect(-cW / 2 + 5, -55 + 5, cW, 110, 16);
                boxBg.fillStyle(0xffffff, 0.9);
                boxBg.lineStyle(4, 0x22c55e, 1);
                boxBg.fillRoundedRect(-cW / 2, -55, cW, 110, 16);
                boxBg.strokeRoundedRect(-cW / 2, -55, cW, 110, 16);
                previewBoxContainer.add(boxBg);

                // const boxTitle = this.add.text(0, -35, '✦ JALUR SAYA ✦', {
                //     fontFamily: '"Press Start 2P", monospace',
                //     fontSize: '8px',
                //     color: '#15803d',
                //     stroke: '#ffffff',
                //     strokeThickness: 2
                // }).setOrigin(0.5);
                // previewBoxContainer.add(boxTitle);

                const previewGroup = createJalurPreview(this, 0, 0, 0.85);
                previewBoxContainer.add(previewGroup);

                // =============================================
                //  RESPONSIVE DOM OVERLAY POSITIONING (PROFIL.GIF)
                // =============================================
                const positionProfilGif = () => {
                    ...
                    // Target frame center is at cardContainer (cx, H/2) - 130 = (cx, H/2 - 130)
                    // Left offset of frame slot is cx - 50. Top offset is H/2 - 130 - 50 = H/2 - 180. Width=100, Height=100.
    ...
    <script>
        (function () {
            ...
            if (avatarEl) {
                if (selectedAvatar === 'profil') avatarEl.src = '../assets/image/ui/profil.gif';
                else if (selectedAvatar === 'profil2') avatarEl.src = '../assets/image/ui/profil2.gif';
                else if (selectedAvatar === 'profil3') avatarEl.src = '../assets/image/ui/profil3.gif';
                else if (selectedAvatar === 'profil4') avatarEl.src = '../assets/image/ui/profil4.gif';
            }
        })();
    </script>
</body>
